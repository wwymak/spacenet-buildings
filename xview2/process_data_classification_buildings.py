...
"""
NOTE::
cut out buildings from the post damage dataset, so the net wouldn't use the surroundings to classify building damage
"""
...
def process_img(img_array, polygon_pts, scale_pct):
    ...
    return (img_array * mask_img)[ymin:ymax, xmin:xmax, :]
...
def process_data(data_dir, image_crops_dir, output_csv_path,):
    ...
    for label_file, img_file in tqdm(zip(labels, images)):
        ...
        for r in label_gdf.iterrows():
            ...
            poly_arr = process_img(img_array, polygon_pts, 0.8)
            poly_img = PIL.Image.fromarray(poly_arr)
